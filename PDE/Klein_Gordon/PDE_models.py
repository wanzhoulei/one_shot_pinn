...
##neural network to solve a series of PDEs of form uxx + utt = f
##and later use perturbation method and one-shot TL to solve the Klein-Gordon Equation 
...
#original shape 2-256-256-256-512-768-(3, 256) - k heads
...
class Multihead3(nn.Module):
  def __init__(self, k, act = nn.Tanh(), bias = False): #silu
    ...
    self.linear5 = nn.Linear(512, 768)
  ...
  def forward(self, x):
    ...
    out = self.act(self.linear5(out)) #out of shape (N, 768)
    out1 = out[:, :256] #shape (N, 256)
    out2 = out[:, 256:512] #shape (N, 256)
    out3 = out[:, 512:] #shape (N, 256)
    output = []
    for i in range(self.k):
      first = self.final_layers[i](out1) #shape (N, 1)
      second = self.final_layers[i](out2) #shape (N, 1)
      third = self.final_layers[i](out3) #shape (N, 1)
      concat = torch.cat((first, second, third), axis=1) #shape (N, 3)
      output.append(concat)
    return torch.stack(output), out #of shape (k, N, 3) and (N, 768) repectively
...
##decorator to return a forcing function of form a sin(pi*k1x)sin(pi*k2t) + b where k1
#is int and k2 is even
##the returned value has the same dimension as the input x and t
def forcing_decorator_trig(a, k1, k2, b=0):
  def force(x, t):
...
def loss(model, interior_grid, x_boundary_num, t_boundary_num, boundary_value,
         Forcing_functions, truth_functions, pde_weight=1, bc_weight=1, data_weight=1, method='chebyshev'):
    ...
    dydt = torch.cat([diff(y[:,i].reshape(-1, 1), t) for i in range(y.shape[1])], dim=1) #shape (N, k)
    dzdx = torch.cat([diff(z[:,i].reshape(-1, 1), x) for i in range(z.shape[1])], dim=1) #shape (N, k)
    ...
    output_boundary, _ = model(input_boundary) #(k, N_boundary, 3)
    u0_boundary = output_boundary[:,:,0] #(k, N_boundary)
    # y0_boundary = output_boundary[:,:,1]
    # z0_boundary = output_boundary[:,:,2]
    # bc_loss = (F.mse_loss(u0_boundary, torch.ones_like(u0_boundary)*boundary_value) +
    #            F.mse_loss(y0_boundary, torch.zeros_like(y0_boundary)) +
    #            F.mse_loss(z0_boundary, torch.zeros_like(z0_boundary)))/3
...
def train(model, optimizer, lossfn, num_iter, Forcing_functions, truth_functions,
          boundary_value, interior_grid=(30, 30), x_boundary_num=100, t_boundary_num=100,
  ...
  for i in tqdm(range(num_iter)):
    ...
    total, pde, bc, data = loss(model, interior_grid, x_boundary_num, t_boundary_num, boundary_value,
                Forcing_functions, truth_functions,
...
def compute_AH(A, H):
  ...
  # Reshape H to (3600, 3, 256) to match the dimensions of A
  H_reshaped = H.reshape(-1, 3, W_size)
...
def compute_H_dict(model, I, B, bias):
  ...
  A1 = np.array([[0, 0, 0], [1, 0, 0], [0, 0, 1]]);
  A2 = np.array([[1, 0, 0], [0, 0, 0], [0, 1, 0]]);
  A3 = np.array([[0, 1, 0], [0, 0, 1], [0, 0, 0]]);
  A1Hx = compute_AH(A1, Hx) #of shape (3I, W)
  A2Ht = compute_AH(A2, Ht) #shape (3I, W)
  A3H = compute_AH(A3, H) #shape (3I, W)
  H_star = A1Hx + A2Ht - A3H #(3I, W)
...
def compute_M(H_dict):
  ...
  H_b_0 = H_dict['H_b'][[3*i for i in range(3*B)]]
  N_i = I**2; N_b = 3*B
...
def compute_TLW(b0, forcing_f, Minv, H_dict, x, t, x_boundary=None, t_boundary=None):
  ##obtain the first rows of H_b
  H_b_0 = H_dict['H_b'][[3*i for i in range(3*H_dict['B'])]]
  ##obtain the forcing fuunction evaluated at interior points of shape (3I, 1)
  f_values = forcing_f(x, t).detach().numpy() #shape (I, 1)
  F_values = np.concatenate([np.zeros((f_values.T).shape), np.zeros((f_values.T).shape), f_values.T]).T.reshape(-1, 1) #(3I, 1)
...
def compute_solution(H, W, I):
  H_ = H[[3*i for i in range(round(H.shape[0]/3))],:]
...
def after_loss(H_dict, W, forcing_f, b0, x, t, x_boundary=None, t_boundary=None):
  ...
  N_b = 3*H_dict['B']
  f_values = forcing_f(x, t).detach().numpy() #shape (I, 1)
  F_values = np.concatenate([np.zeros((f_values.T).shape),
                            np.zeros((f_values.T).shape), f_values.T]).T.reshape(-1, 1) #(3I, 1)
  ...
  boundary_values = (H_b@W)[[3*i for i in range(N_b)]]
...
def KG_solver(H_dict, Minv, f0_values, b, landa, H, p=11):
  t0 = time.time();
  ##compute the BC condition on each PDE of the series
  b_each = compute_each_b(b, landa, p)
  ...
  W = combine_W(W_list, landa)
  ##compute the solution
  sol = compute_solution(H, W, I=round((H.shape[0]/3)**.5))
...
##this function compute the Klein-Gordon Loss
##returns a  dictionary that contains: 1. total loss 2. PDE loss, 3. BC loss
def KG_loss(model, W, landa, forcing_function, b0, I=60, B=200, bias=True):
  x, t, interior_tensor = generate_interior_tensor(I=I, require_grad = True)
  ##define the boundary set used to do transfer learing on cpu
  ...
  ##detach the H
  H = H.reshape(3*H.shape[0], -1) #transform all H into shape (3I^2, W)
  H = H[[3*i for i in range(I**2)]] ##only pick the entries that corresponds to u
  ...
  utt = diff(diff(u, t), t) ##(I**2, 1)

  f_term = forcing_function(x, t)
  residual = uxx + utt + landa*u**2 - f_term
  pde_loss = F.mse_loss(residual, torch.zeros_like(residual)).item()

  ##compute the boundary loss now
  _, H0  = model(boundary_tensor)
  ##detach the H0
  H0 = H0.reshape(3*H0.shape[0], -1) #transform all H into shape (3*3*B, W)
  H0 = H0[[3*i for i in range(3*B)]] ##only pick the entries that corresponds to u
  if bias: H0 = torch.hstack((H0, torch.ones((H0.shape[0], 1))))

  ##compute the u0 = H0W
  u0 = H0@torch.tensor(W) ##(3*B, 1)
